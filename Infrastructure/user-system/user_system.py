...
# Import from Core modules (relative to bot.py location)
import db
import config
...
class UserSystem:
    ...
    async def _sync_banned_users_from_config(self) -> None:
        ...
        for user_id in config.BANNED_USER_IDS:
            uid_str = str(user_id)
    ...
    async def _is_banned(self, user_id: int) -> dict[str, Any] | None:
        ...
        record = await db.get_ban(user_id)
